...
DEFAULT_SAXON_EXECUTABLE = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "resources", "saxon9", "saxon9he.jar"
)
...
def add_sample_metadata(sample_info_file, input_study_file):
    ...
    result = result[
        [
            "Source Name",
            "Material Type",
            "Characteristics[barcode identifier]",
            "internal_ID",
            "resolute_ID",
            ...
            "cellLine",
            "cellosaurusID",
            "Characteristics[chemical compound]",
            "Protocol REF_x",
            "Date",
            "Sample Name",
            ...
            "Factor value [cellNumber]",
            "Factor value [replicate]",
            "Factor value [extractionVolume, µl]",
        ]
    ]

    result["cellosaurusID"] = result["cellosaurusID"].str.replace(
        "cellosaurus:CVCL_", " https://web.expasy.org/cellosaurus/CVCL_"
    )

    result = result.rename(
        columns={
            "internal_ID": "Characteristics[internal_ID]",
    ...
    result = result.rename(
        columns={
            "Term Source REF.1": "Term Source REF",
