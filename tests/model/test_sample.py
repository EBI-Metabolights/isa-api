...
class TestSample(TestCase):
    ...
    def test_init(self):
        ...
        sample = Sample(
            name="sample1", factor_values=[factor_value], characteristics=[characteristic], derives_from=[source]
        )
    ...
    def test_repr(self):
        expected_str = (
            "isatools.model.Sample(name='', characteristics=[], factor_values=[], derives_from=[], comments=[])"
        )
    ...
    def test_to_dict(self, mock_uuid):
        ...
        expected_dict["characteristics"] = [
            {"category": {"@id": "#characteristics/0"}, "value": None, "unit": {"@id": "#unit/0"}, "comments": []}
        ]
        self.assertEqual(self.sample.to_dict(), expected_dict)
        ...
        expected_dict["factorValues"] = [
            ...
            {"category": {"@id": "#factor/1"}, "value": "", "unit": {"@id": "#unit/" + mock_uuid.return_value}},
        ]
        self.assertEqual(self.sample.to_dict(), expected_dict)

        self.sample.derives_from = [Source(name="source0", id_="#source/0"), Source(name="source1", id_="#source/1")]
    ...
    def test_from_dict(self):
        ...
        expected_dict["characteristics"] = [{"category": {"@id": "cat_id"}, "comments": [], "value": "val"}]
        sample.from_dict(expected_dict)
        self.assertEqual(sample.to_dict(), expected_dict)
        ...
        expected_dict["factorValues"] = [{"category": {"@id": "factor0"}, "value": ""}]
        sample.from_dict(expected_dict)
        ...
        indexes.sources = {"my_source": Source(id_="my_source")}
