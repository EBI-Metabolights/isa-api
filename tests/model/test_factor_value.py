...
class TestStudyFactor(TestCase):
    ...
    def test_factor_type(self):
        ...
        self.assertTrue(
            "StudyFactor.factor_type must be a OntologyAnnotation or None; got 1:<class 'int'>"
            in str(context.exception)
        )

    def test_repr(self):
        expected_repr = (
            "isatools.model.StudyFactor(name='', factor_type=isatools.model.OntologyAnnotation(term='', "
...
class TestFactorValue(TestCase):
    def setUp(self):
        self.factor_value = FactorValue(
            factor_name=StudyFactor(name="Control"), value=12, unit=OntologyAnnotation(term="mg")
        )
    ...
    def test_factor_name(self):
        ...
        self.assertTrue(
            "FactorValue.factor_name must be a StudyFactor or None; got 1:<class 'int'>" in str(context.exception)
        )
    ...
    def test_value(self):
        ...
        self.assertTrue(
            "FactorValue.value must be a string, numeric, an OntologyAnnotation, or None; "
    ...
    def test_unit(self):
        ...
        self.assertTrue(
            "FactorValue.unit must be an OntologyAnnotation, o string, or None; got 1:<class 'int'>"
            in str(context.exception)
        )
    ...
    def test_repr(self):
        self.maxDiff = None
        factor_name_str = (
            "isatools.model.StudyFactor(name='Control', factor_type=isatools.model.OntologyAnnotation("
        ...
        expected_str = "isatools.model.FactorValue(factor_name={0}, value=12, unit={1})".format(
            factor_name_str, unit_str
        )
    ...
    def test_equalities(self):
        second_factor_value = FactorValue(
            factor_name=StudyFactor(name="Control"), value=12, unit=OntologyAnnotation(term="mg")
        )
        third_factor_value = FactorValue(
            factor_name=StudyFactor(name="Control"), value=13, unit=OntologyAnnotation(term="mg")
        )
    ...
    def test_to_dict(self, mock_uuid):
        first_factor_value = FactorValue(
            factor_name=StudyFactor(name="test_factor_name", id_="#factor/0"),
            value=OntologyAnnotation(term="test_value", id_="#factor_value/0"),
            unit=OntologyAnnotation(term="test_unit", id_="#unit/0"),
        )
        second_factor_value = FactorValue(factor_name=StudyFactor(name="factor_name1", id_="#factor/1"), unit="unit1")
        expected_dict = {
